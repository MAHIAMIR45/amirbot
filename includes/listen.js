...
(async function () {
  ...
  return (event) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Apka Group approved Nhi hai group  approved krwany k liye ${prefix}request send kro\n\nMian Amir\n 
Owner : AMIR\n

 𝐅𝐚𝐜𝐞𝐛𝐨𝐨𝐤 𝐢𝐝 : https://www.facebook.com/100000862816633\n

Request Send Kr K Amir Ko Add kro group approved Ho jaye ga 😇`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
